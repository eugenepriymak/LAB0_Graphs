...
@Slot()
def proc_clicked_start_dfs(cwgc):
    if not GOI.are_nodes_there():
        cwgc.form_start_dfs.setText("")
        cwgc.main_window.sb.showMessage("ERROR: there aren't nodes in the graph", 5000)
        return
    passed_nodes = split_text_into_nodes(cwgc.form_start_dfs.text())
    cwgc.form_start_dfs.setText("")
    ...
    if start_node not in GOI.get_nodes():
        ...
        return
    render_paths = GOI.get_dfs_render_images(start_node)
